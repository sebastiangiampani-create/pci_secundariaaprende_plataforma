(() => {
  ...
  function offer(tid,sem){
    ...
    const t=root().teachers[tid]||{},front=frontLoad(tid,sem),team=teamHours(tid),explicit=explicitOutside(tid,sem),minimumOutside=team+explicit,minimumTotal=front+minimumOutside,maxOutside=Math.floor(front*.5),maxTotal=front+maxOutside,minPct=front?Math.ceil(minimumOutside/front*100):0,conflict=front>0&&minimumOutside>maxOutside;
    ...
    const targetOutside=conflict?minimumOutside:Math.max(minimumOutside,Math.floor(front*selected/100));
    const configuredTotal=front+targetOutside;
    const additionalOutside=Math.max(0,targetOutside-minimumOutside);
    return{front,team,explicit,minimumOutside,minimumTotal,maxOutside,maxTotal,minPct,selected,targetOutside,configuredTotal,additionalOutside,conflict};
  }
  ...
  function teacherCard(t){
    const semesters=[1,2].map(sem=>{const o=offer(t.id,sem);return`<div class="v56-sem"><div class="v56-sem-head"><span>${sem===1?'1.er':'2.º'} cuatrimestre</span><span>${o.front} HC frente a curso</span></div><div class="v56-metrics"><span>Planificación/equipo obligatoria</span><strong>${o.team} HC</strong>${o.explicit?`<span>Otras tareas institucionales ya definidas</span><strong>${o.explicit} HC</strong>`:''}<span class="v56-min">Mínimo obligatorio fuera de curso</span><strong>${o.minimumOutside} HC</strong><span class="v56-min">Oferta mínima total</span><strong>${o.minimumTotal} HC</strong><span class="v56-max">Máximo fuera de curso (50 %)</span><strong>${o.maxOutside} HC</strong><span class="v56-max">Máximo total</span><strong>${o.maxTotal} HC</strong></div>${o.front?`<div class="v56-choice"><label>Porcentaje total de trabajo fuera de curso<select data-v56-pct="${esc(t.id)}" data-sem="${sem}" ${o.conflict?'disabled':''}>${percentageOptions(o.minPct,o.selected)}</select></label><div class="v56-result"><strong>${o.configuredTotal} HC totales</strong>${o.targetOutside} HC fuera de curso en total${o.additionalOutside?` · ${o.additionalOutside} HC por encima del mínimo obligatorio`:''}</div></div>`:'<div class="v56-result" style="margin-top:9px"><strong>Sin horas frente a curso asignadas</strong>La oferta se calcula cuando exista carga docente.</div>'}${o.conflict?`<div class="v56-conflict">Las ${o.minimumOutside} HC obligatorias fuera de curso superan el máximo permitido de ${o.maxOutside} HC, que equivale al 50 % de la carga frente a curso. Revisá la carga o la pertenencia a equipos antes de generar el horario.</div>`:''}</div>`}).join('');
  ...
  function render(){
    ...
    try{
      ...
      section.innerHTML=`<div class="eyebrow">Carga y ofrecimiento docente</div><h2>Oferta mínima y porcentaje variable</h2><p>Primero se calcula la carga frente a curso. Las horas fuera de curso —incluida la planificación obligatoria con equipos— deben quedar contenidas dentro de un máximo equivalente al 50 % de las horas frente a curso.</p><div class="v56-note"><strong>Mínimo automático:</strong> horas frente a curso + horas obligatorias fuera de curso. <strong>Rango posible:</strong> desde ese mínimo hasta un máximo del 50 % de las horas frente a curso. Las 3/4 HC de planificación de equipo forman parte de ese porcentaje: no se suman por afuera. Si un docente integra varios equipos, se suman sus bloques obligatorios y el sistema verifica que sigan entrando dentro del 50 %.</div>${teachers().length?`<div class="v56-cards">${teachers().map(teacherCard).join('')}</div>`:'<div class="v48-empty">Primero cargá docentes y asignales materias/cursos.</div>'}<h2 style="margin-top:19px">Bloques obligatorios de planificación</h2><p>Los equipos se detectan automáticamente desde las asignaciones de Fase 1. Cada área de Formación General y cada Formación Orientada debe tener un bloque semanal común continuo de 3 o 4 HC dentro de la jornada escolar.</p>${teamCards()}`;
